...
void normalizeCompactCommand(String &commandLine) {
  commandLine.trim();

  if (commandLine.length() < 2 || commandLine.indexOf(' ') >= 0) {
    return;
  }

  char mode = toupper(commandLine.charAt(0));

  if (mode == 'M' || mode == 'N' || mode == 'R') {
    commandLine = String(mode) + " " + commandLine.substring(1);
  }
}

void processCommand(String commandLine) {
  normalizeCompactCommand(commandLine);
...
String normalizeCommandWithRepeats(String command, String repeats) {
  normalizeCompactCommand(command);
  command.trim();
  repeats.trim();
...
bool isSingleKeyShortcut(const String &shortcut) {
  return shortcut.length() == 1;
}

bool sendCustomShortcut(char key) {
  for (uint8_t i = 0; i < customCommandCount; i++) {
    if (isSingleKeyShortcut(customCommands[i].shortcut) && customCommands[i].shortcut.charAt(0) == key) {
...
void sendWebPage() {
  ...
  if (customCommandCount == 0) {
    ...
    for (uint8_t i = 0; i < customCommandCount; i++) {
      ...
      page += "'" + jsEscape(customCommands[i].shortcut) + "':'" + jsEscape(customCommands[i].command) + "'";
      wroteShortcut = true;
    }
  ...
  page +=
    ";const keyName=e=>{let k=e.key.length===1?e.key.toLowerCase():e.key;let parts=[];if(e.ctrlKey)parts.push('ctrl');if(e.altKey)parts.push('alt');if(e.shiftKey&&e.key.length===1)parts.push('shift');parts.push(k);return parts.join('+')};document.addEventListener('keydown',e=>{if(e.target.matches('input,textarea,select,button'))return;const name=keyName(e);const cmd=shortcuts[name]||shortcuts[e.key]||shortcuts[e.key.toLowerCase()];if(!cmd)return;e.preventDefault();saveScroll();location.href='/send?cmd='+encodeURIComponent(cmd);});</script>"
    "</main></body></html>";
